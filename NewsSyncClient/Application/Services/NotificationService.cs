...
public class NotificationService : INotificationService
{
    ...
    public Task<List<NotificationDto>> GetNotificationsAsync()
    {
        if (string.IsNullOrWhiteSpace(_session.UserId))
            throw new UserInputException("You must be logged in to fetch notifications.");
    ...
    {
        if (string.IsNullOrWhiteSpace(_session.UserId))
            throw new UserInputException("You must be logged in to configure notifications.");
